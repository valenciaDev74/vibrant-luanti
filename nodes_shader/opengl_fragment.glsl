...
#if 0
// Disabled: water reflection noise functions
vec4 perm(vec4 x)
{
...
void main(void)
{
	...
	if (f_shadow_strength > 0.0) {
		...
		vec3 night_part = mix(col.rgb, night_ambient, darkness * ambientWeight * ambientWeight);

		col.rgb = adjusted_night_ratio * night_part + (1.0 - adjusted_night_ratio) * day_part;
		...
		// Disabled: water reflections (sky + sun reflection on water surfaces)
#if 0
#if (defined(ENABLE_WATER_REFLECTIONS) && MATERIAL_WATER_REFLECTIONS && ENABLE_WAVING_WATER)
		...
		// Sun reflection highlight (stronger on water)
		float spec_angle = pow(max(dot(reflect_ray, viewVec), 0.0), 4.0);
		float fresnel = clamp(pow(1.0 - abs(dot(viewVec, fNormal)), 3.0), 0.3, 1.0);
#ifdef MATERIAL_WATER_REFLECTIONS
		float spec_intensity = MATERIAL_WATER_REFLECTIONS != 0 ? 4.0 : 1.0;
		if (MATERIAL_WATER_REFLECTIONS != 0) {
		...
		ref_color *= max(1.0 - shadow_uncorrected, 0.0);
		ref_color *= f_adj_shadow_strength;
		ref_color *= 1.0 - adjusted_night_ratio;
		col.rgb += ref_color;
